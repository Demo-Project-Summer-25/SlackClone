...
public interface DirectConversationRepository extends JpaRepository<DirectConversation, Long> {
    ...
    @Query("select dp.conversationId from DirectParticipant dp where dp.userId = :userId and dp.leftAt is null")
    List<Long> findActiveConversationIdsByUser(@Param("userId") Long userId);
    ...
    List<DirectConversation> findAllByIds(@Param("ids") List<Long> ids);
